...
def create_weather_agent() -> Agent:
    """Create the weather agent with all tools and memory hooks"""
    current_date = datetime.utcnow().strftime("%B %d, %Y")
    
    system_prompt = f"""You are a Weather-Based Activity Planning Assistant with memory capabilities.

    CRITICAL DATE CONTEXT: 
    - Today is {current_date} (Friday)
    - This weekend is November 29-30, 2025
    - ALL weather forecasts must be for late November or December 2025
    - If you receive weather data with dates from 2024 or early 2025, reject it as invalid and request new data
    ...
    7. Generate Activity Recommendations based on weather and preferences that you have recieved from previous steps
    8. Generate the comprehensive Markdown file and store it in S3 Bucket: {RESULTS_BUCKET}
       - Use the filename from step 6 (format: {{city_name}}_{{YYYY-MM-DD}}.md)
       - Use the use_aws tool to upload the file to S3 with this filename 
